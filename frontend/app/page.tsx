  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const detail = axios.isAxiosError(unknownError)
        ? (unknownError.response?.data as { detail?: string } | undefined)
            ?.detail
        : undefined;
    ...
    <main className="screen-shell">
      <section className="panel panel-compact upload-panel">
        ...
        <h1 className="heading-xl">
          Real-Time Results Ingestion and Reporting
        </h1>
        <p className="muted">
          Import rolling constituency files, apply deterministic override rules,
          and track seat and vote totals from a single dashboard.
        </p>
        ...
        {response && (
          <div className="import-summary">
            ...
            {response.errors.length > 0 && (
              <p className="muted">
                Warnings captured: {response.errors.length} (see API response
                for details)
              </p>
            )}
            <button
